...
namespace MikeNspired.XRIStarterKit {
    ...
    /// </summary>
    public class XRToggleSlider : UnityEngine.XR.Interaction.Toolkit.Interactables.XRBaseInteractable {
        ...
        // --- Nuevos eventos para máximo y mínimo ---
        [Header("On Min/Max Events")]
        [SerializeField]
        [Tooltip("Invoked once when the throttle reaches (or exceeds) the maximum value")]
        UnityEvent m_OnMaxValue = new UnityEvent();

        [SerializeField]
        [Tooltip("Invoked once when the throttle reaches (or goes below) the minimum value")]
        UnityEvent m_OnMinValue = new UnityEvent();

        [SerializeField]
        [Tooltip("Tolerance used to decide if value is considered Min or Max (to avoid float issues)")]
        float m_MinMaxThreshold = 0.001f;

        // Flags para evitar invocaciones repetidas
        ...
        void SetValue(float value) {
            ...
            // --- Manejo de eventos de Min/Max (transiciones) ---
            // Detectar Max
            if (m_Value >= 1.0f - m_MinMaxThreshold) {
                if (!m_HasReachedMax) {
                    m_HasReachedMax = true;
                    m_HasReachedMin = false; // salir de min si venías de ahí
                    m_OnMaxValue?.Invoke();
                }
            }
            // Detectar Min
            else if (m_Value <= 0.0f + m_MinMaxThreshold) {
                if (!m_HasReachedMin) {
                    m_HasReachedMin = true;
                    m_HasReachedMax = false; // salir de max si venías de ahí
                    m_OnMinValue?.Invoke();
                }
            }
            else {
                // Si estás en zona intermedia, resetear flags para permitir futuras transiciones
